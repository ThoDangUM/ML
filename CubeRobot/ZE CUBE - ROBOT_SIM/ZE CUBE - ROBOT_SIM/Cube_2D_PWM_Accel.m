% This function simulates 2D Cube robot with PWM input
%clear all
%clc
%close all
function ETAT_SAVE = Cube_2D_PWM_Accel(PWM)
...
%pause(10);
%
%fileRobot = fopen('Robot_data_20_05_22.txt','w');
%fileID = fopen('Cm_Data_19_05_22.txt','r');
%f_size = 194481;
% read PWM value
...
%    ETAT = [X,];% x y psi u v r u_dot v_dot r_dot
    [ETAT_NEW,F_B_Effective,F_M_Effective,ACCEL_eff]=Modele_Dynamique_Integration_2D_acceleration(ETAT,Concentrateur,PWM,Dyn_Robot,Moteur,dt);
    Q_B_0 = ETAT_NEW(1:4);
...
%        ETAT_SAVE = [V_0(1), V_0(2), W_B(3)];% u v r
        ETAT_SAVE = ACCEL_eff;% u_dot, v_dot, r_dot
