%PARAMETRES DU CUBE in 2D
clear all
d=0.27;
%%%% MOTEURS %%%%
Moteur(1).position=[d d];
Moteur(2).position=[d -d];
Moteur(3).position=[-d -d];
Moteur(4).position=[-d d];
% Moteur(5).position=[-d -d -d];
% Moteur(6).position=[-d d -d];
% Moteur(7).position=[-d d d];
% Moteur(8).position=[-d -d d];

Moteur(1).orientation=[0];
Moteur(2).orientation=[-pi/2];
Moteur(3).orientation=[pi];
Moteur(4).orientation=[pi/2];
% Moteur(5).orientation=[0    0    pi/2];
% Moteur(6).orientation=[0    -pi/2    0];
% Moteur(7).orientation=[0    0    pi];
% Moteur(8).orientation=[0    pi/2    0];

Moteur(1).Max_pos=100;
Moteur(2).Max_pos=100;
Moteur(3).Max_pos=100;
Moteur(4).Max_pos=100;
% Moteur(5).Max_pos=39;
% Moteur(6).Max_pos=35;
% Moteur(7).Max_pos=40;
% Moteur(8).Max_pos=37;

Moteur(1).Max_neg=-100;
Moteur(2).Max_neg=-100;
Moteur(3).Max_neg=-100;
Moteur(4).Max_neg=-100;
% Moteur(5).Max_neg=-22;
% Moteur(6).Max_neg=-21;
% Moteur(7).Max_neg=-17;
% Moteur(8).Max_neg=-20;
...
%save PARAM_Moteurs_CUBE_2D Moteur Moteur_EST
save PARAM_Moteurs_CUBE_2D Moteur
...
Dyn_Robot.m_u=10;Dyn_Robot.m_v=10;Dyn_Robot.m_r=1;
Dyn_Robot.d_u=1;Dyn_Robot.d_v=1;Dyn_Robot.d_r=1;

%Dyn_Robot_EST = Dyn_Robot;

%save PARAM_Dyn_CUBE_2D Dyn_Robot Dyn_Robot_EST
save PARAM_Dyn_CUBE_2D Dyn_Robot
